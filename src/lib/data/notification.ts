    ...
    await dbInstance
      ...
      .where(
        and(
          ...
          params.commentId ? eq(notifications.commentId, params.commentId) : undefined
        )
      );
  } catch (error) {
...
function refresh(params : manageNotificationParams) {
  if (params.type === "LIKE" && params.postId) {
    revalidatePath(`/post/${params.postId}`);
  } else if (params.type === "COMMENT" && params.postId) {
    revalidatePath(`/post/${params.postId}`);
  } else if (params.type === "FOLLOW" && params.actorId && params.recipientId) {
    revalidatePath(`/profile/${params.actorId}`);
    revalidatePath(`/profile/${params.recipientId}`);
  } else if (params.type === "REPLY" && params.postId) {
    revalidatePath(`/post/${params.postId}`);
  } else if (params.type === "COMMENT_LIKE" && params.postId) {
    revalidatePath(`/post/${params.postId}`);
  }
}